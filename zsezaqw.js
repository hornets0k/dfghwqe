...
client.on('voiceStateUpdate', (oldMember, member) => {
	...
	if (member.voiceChannelID) {
		...
		if (newChannel.name.startsWith(String.fromCodePoint('0x1F3AE'))) {
			newChannel.clone(String.fromCodePoint('0x2501') +'в РљРѕРјР°РЅРґР° ', true)
				.then(createdChannel => {
					createdChannel.edit({
							bitrate: 48000,
							position: newChannel.position + 1100,
							userLimit: newChannel.userLimit + 4
						})
						.then(createdChannel => {
	...
	if (oldMember.voiceChannelID) {
		...
		if (oldChannel.name.startsWith(String.fromCodePoint('0x2501') +'в РљРѕРјР°РЅРґР°') && !oldChannel.members.array().length) {
			oldChannel.delete()
				.then(function() {
...
client.on('channelCreate', function(channel){
	if(!channel.name.startsWith(String.fromCodePoint('0x2501') +'в РљРѕРјР°РЅРґР°')){
		orderChannels();
	}
});
...
function orderChannels(){
	...
	channelsOrdered.forEach(function(channel) {
		...
		currentChannel.edit({bitrate: 64000, position: currentPosition})
			.then(editedChannel => {
...
client.login("NDM0NTA5MTMzNzY1NjcyOTYw.DbL0Kw.su2p6iue-ns4ffmuaNvP44ZfLhQ");
